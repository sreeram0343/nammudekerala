...
def seed_if_empty():
    """
    Checks if the Assembly table is empty. If so, seeds the initial 140 constituencies.
    This is designed to help users on free-tier hosting (like Render Free) who 
    cannot access the shell to run manual seed scripts.
    """
    db = SessionLocal()
    try:
        count = db.query(Assembly).count()
        if count > 0:
            logger.info(f"Database already contains {count} assemblies. Skipping auto-seed.")
            return

        logger.info("Database is empty. Starting automatic seeding of Kerala constituencies...")
        ...
        db.commit()
        logger.info("Successfully seeded all 140 Kerala constituencies.")
    except Exception as e:
        db.rollback()
        logger.error(f"Error during automatic seeding: {e}")
    finally:
        db.close()
